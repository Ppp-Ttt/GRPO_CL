...
## 数据集下载
```
hf download meetforever/GRPO_CL --repo-type dataset --local-dir ./data
```

## base模型下载
```
hf download Qwen/Qwen3-8B --local-dir ./basemodel/Qwen3-8B
```

## 训练脚本
训练脚本位于```scripts_train```文件夹下，可能需要修改的参数：
```
...
# 以下参数影响显存占用和训练速度，按需选择 
actor_rollout_ref.model.enable_gradient_checkpointing=True \ # 好像需要与下面一项相同
actor_rollout_ref.model.enable_activation_offload=True \ # 设置为False提速
actor_rollout_ref.actor.fsdp_config.param_offload=True \  # 设置为False提速
actor_rollout_ref.actor.fsdp_config.optimizer_offload=True \  # 设置为False提速
actor_rollout_ref.rollout.tensor_model_parallel_size=1 \  # 可设置为2提速
actor_rollout_ref.ref.fsdp_config.param_offload=False \  # 设置为False提速
```

默认启用```wandb```进行训练监控（在脚本中```trainer.logger='["console","wandb"]'```设置），需登录wandb账号
...
## 测试
在```scripts_eval```中运行测试脚本，需要修改模型权重路径，以及测试数据文件路径。

## 其它
verl中文文档 https://woniu9524.github.io/verl-doc/index.html
verl官方文档(en) https://verl.readthedocs.io/en/latest/index.html
